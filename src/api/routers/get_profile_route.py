...
def verify_token(token: str) -> int:
    try:
        payload = jwt.decode(token, api.settings.SECRET_KEY, algorithms=[api.settings.ALGORITHM])
...
def get_profile(request : Request, access_token: str = Cookie(None), refresh_token : str = Cookie(None)):
    ...
    if code == 2:
        ...
        try:
            payload = jwt.decode(refresh_token, api.settings.SECRET_KEY, algorithms=[api.settings.ALGORITHM])
