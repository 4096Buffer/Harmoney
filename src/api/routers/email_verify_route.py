...
@router.get("/")
def verify_email(request: Request, uid: int = Depends(get_user_id_from_token), mode: int = 0, token: str = ""):
    ...
    user_tokens = database.Get("SELECT * FROM email_verify_tokens WHERE user_id = :id", { "id" : uid })
    user_token = None
    ...
    if mode == 0:
        ...
        database.Insert({"token" : gen_token, "expires_at" : expires_at, "user_id" : uid, "active" : True}, "email_verify_tokens")

        emailer.send_email(row['email'], "Harmoney - Verify your E-mail", f'<a href="http://127.0.0.1:8000/email-verify?token={gen_token}&mode=1">Click here to verify -> Verify</a>')
